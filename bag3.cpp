...
using namespace std;
	...
	void bag::operator -=(const bag& subtractor) {	
		if (subtractor.many_nodes > 0) {
			for (node *subtractor_cursor = subtractor.head_ptr; subtractor_cursor != NULL; subtractor_cursor = subtractor_cursor -> link()){
				node *temp_head = head_ptr;
				node *previous = NULL;
				while (temp_head != NULL){
					if (temp_head -> data() == subtractor_cursor -> data()){
						if (previous == NULL){
							list_head_remove(head_ptr);
							many_nodes--;
							break;
